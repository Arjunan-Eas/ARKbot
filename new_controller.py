...
def drive_forward(value):
    ...
    received_string = read_float_from_device(UPPER_CONTROLLER)
    print(f"Distance: {received_string}")

    if float(received_string) > 40:
...
def open_claw():
    send_to_upper("openClaw,200,100")

def close_claw():
    send_to_upper("closeClaw,200,10000")

def arm_up():
    send_to_upper("armUp,255,150")

def arm_down():
    send_to_upper("armDown,150,100")
